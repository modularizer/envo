...
def run_interactive_config(
    ...
    state = {
        ...
        'last_arrow_time': 0.0,  # For debouncing scroll-generated arrow events
    }
    
    # Minimum time between arrow key events (seconds)
    # 30ms filters rapid scroll bursts but allows normal key repeat
    ARROW_DEBOUNCE_SECONDS = 0.03
            ...
            if state['edit_mode']:
                ...
                state['cached_layout'] = Layout(
                    HSplit([
                        ...
                        detail_window,
                        Window(height=1),
                        Window(height=1, content=FormattedTextControl(lambda: [(UI_TEXT_DIM, "j/k or ↑↓ Navigate  Enter Edit  Ctrl+S Save  Ctrl+Q Quit")])),
                    ]),
    ...
    # Vim-style navigation (j/k) - these are never generated by scroll
    @kb.add('k', filter=is_not_edit_mode)
    def go_up_vim(event):
        """Move selection up (vim style)."""
        _navigate_up()
        event.app.invalidate()
    
    @kb.add('j', filter=is_not_edit_mode)
    def go_down_vim(event):
        """Move selection down (vim style)."""
        _navigate_down()
        event.app.invalidate()
    
    # Arrow keys with aggressive debouncing to filter scroll-generated events
    @kb.add('up', filter=is_not_edit_mode)
    def go_up(event):
        """Move selection up, with debouncing to filter scroll-generated events."""
        now = time.monotonic()
        elapsed = now - state['last_arrow_time']
        if elapsed < ARROW_DEBOUNCE_SECONDS:
            return  # Ignore rapid events (likely from mouse scroll)
        state['last_arrow_time'] = now
        _navigate_up()
        event.app.invalidate()
    
    @kb.add('down', filter=is_not_edit_mode)
    def go_down(event):
        """Move selection down, with debouncing to filter scroll-generated events."""
        now = time.monotonic()
        elapsed = now - state['last_arrow_time']
        if elapsed < ARROW_DEBOUNCE_SECONDS:
            return  # Ignore rapid events (likely from mouse scroll)
    ...
    # Enable mouse support so we receive proper ScrollUp/ScrollDown events
    # instead of the terminal converting them to arrow keys
    app = Application(
        layout=get_layout(),
        key_bindings=kb,
        full_screen=True,
        mouse_support=True,  # Enable to capture scroll events properly
        enable_page_navigation_bindings=False,  # Disable page up/down
    )
